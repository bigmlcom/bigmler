...
Also, the ``--model-tag`` can be used as well to retrieve multi-labeled
...
This would generate the models needed to predict ``type``. Its important to
remark that the models used to predict ``class`` in the first example will
use the rest of fields (including the ones generated by expanding ``type``
and ``type`` itself) to build the prediction tree. If you don't want this
