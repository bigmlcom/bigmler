...
class TestPrediction(object):
    ...
    def test_scenario01(self):
        ...
        examples = [
            ['data/iris.csv', 'data/test_iris.csv', 'scenario4_fs/predictions.csv', 'check_files/predictions_iris_fs.csv']]
        for example in examples:
            print "\nTesting with:\n", example
            fs_pred.i_create_fs_resources_from_model(self, train=example[0], test=example[1], output=example[2])
            test_pred.i_check_create_model(self)
            dn_pred.i_check_create_deepnet(self)
    ...
    def test_scenario02(self):
        ...
        examples = [
            ["data/iris.csv", 'data/test_iris.csv', 'scenario5_fs/predictions.csv', 'check_files/predictions_iris_fs.csv']]
        for example in examples:
            print "\nTesting with:\n", example
            fs_pred.i_create_fs_resources_from_model_remote(self, train=example[0], test=example[1], output=example[2])
            test_pred.i_check_create_model(self)
            dn_pred.i_check_create_deepnet(self)
            fs_pred.i_check_create_fusion(self)
    ...
    def test_scenario03(self):
        ...
        examples = [
            ["data/iris.csv", 'data/test_iris.csv', 'data/batch_output.json', 'scenario6_fs/predictions.csv', 'check_files/predictions_iris_fs_prob.csv']]
        for example in examples:
            print "\nTesting with:\n", example
            fs_pred.i_create_fs_resources_from_model_remote_with_options(self, train=example[0], test=example[1], output=example[3], options_file=example[2])
            test_pred.i_check_create_model(self)
            dn_pred.i_check_create_deepnet(self)
            fs_pred.i_check_create_fusion(self)
            batch_pred.i_check_create_batch_prediction(self)
