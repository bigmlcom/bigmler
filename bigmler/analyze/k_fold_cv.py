...
CANDIDATES_LOG = u"random_candidate_sets.csv"
...
def create_features_analysis(args, api, common_options, resume=False):
    ...
    model_fields = best_first_search(
        datasets_file, api, args, common_options,
    ...
    bigmler_command = (u'bigmler delete --from-dir %s') % (
        output_dir)
    message = (u'To delete all the resources generated by this analyze'
               u' subcommand use:\n%s\n\n') % bigmler_command
    u.log_message(message, log_file=session_file, console=1)
...
def create_nodes_analysis(args, api, common_options, resume=False):
    ...
    node_threshold = best_node_threshold(
        datasets_file, args, common_options,
        staleness=args.staleness,
        penalty=args.penalty,
        resume=resume)
    # showing the instruction to create the complete model with the
    # chosen node threshold
    bigmler_command = (u'bigmler --dataset %s --node-threshold %s') % (
        args.dataset, node_threshold)
    message = (u'To create the final model with the entire dataset using '
               u'the selected maximum number of nodes use:\n%s\n\n' %
               bigmler_command)
    u.log_message(message, log_file=session_file, console=1)
    # showing the instruction to delete the user-given output-dir
    bigmler_command = (u'bigmler delete --from-dir %s') % (
...
def candidates_evaluate(datasets_file, args, random_candidates,
                        common_options, penalty=DEFAULT_CANDIDATES_PENALTY,
                        metric=ACCURACY, resume=False):
...
def create_candidates_evaluations(datasets_file, args, common_options,
                                  resume=False,
